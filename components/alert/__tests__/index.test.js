...
describe('<Alert/>', () => {
	...
	it('should find cuke-alert classnames', () => {
		const wrapper = render(
			<div>
				...
				<Alert type="success" message="一等奖" description="获得5元兰博基尼购车优惠券"/>
				<Alert type="success" message="一等奖" closable description="获得5元兰博基尼购车优惠券"/>
			</div>
		);
	...
	it('should can trigger onClose event', () => {
		...
		const wrapper = shallow(
			<Alert type="error" showIcon closable  message="有一个 bug?" onClose={()=> wrapper.setState({message:"关闭"})}/>
		);
